...
def calculate_spot_savings(on_demand_price: float, spot_price: float) -> float:
    """
    Calculate savings percentage when using spot instances.
    
    Args:
        on_demand_price: On-demand hourly price
        spot_price: Spot hourly price
...
def project_monthly_cost(hourly_cost: float, hours_per_day: float = 24) -> float:
    """
    Project monthly cost from hourly rate.
    
    Args:
        hourly_cost: Cost per hour
        hours_per_day: Expected hours of usage per day
        
    Returns:
        Projected monthly cost (assuming 30 days)
    """
